window.__createNewLinkTag = function(){
  const tag = document.createElement('hans-tag')
  tag.setAttribute('hans-tag-id', 'HANS-LINKS')
  document.body.appendChild(tag)
  const main = tag.querySelector('.tag-container')
  const detail = document.createElement('details')
  const summary = document.createElement('summary')
  summary.innerText = 'EDIT LINK'
  detail.appendChild(summary)


  const qr_code = document.createElement('qr-code')
  
  qr_code.setAttribute('value', `https://dataroom.network/${window.location.pathname.slice(1)}?&referer=${HANS_ID}`)

  detail.appendChild(qr_code)
...
/*
  ...
  HANS Link, takes a value in attributes and generates a QR code and link to press. 
  
  qr code can hold 1k of string data before it errors out. 

  For Reader: https://blog.minhazav.dev/HTML5-QR-Code-scanning-launched-v1.0.1/
